...
export default function NewMusic({
  ...
  const handleSelectGenre = (genreName: string) => {
    const updatedGenres = musicData.genres.includes(genreName)
      ? musicData.genres.filter((g) => g !== genreName) // Remove genre
      : musicData.genres.length < 3
        ? [...musicData.genres, genreName] // Add genre if limit is not reached
        : musicData.genres;
  ...
  return (
    <div
      ...
      <div className="flex flex-col w-min max-w-1/3">
        <span className="font-medium text-sm inline-flex text-dark-custom truncate overflow-hidden whitespace-nowrap">
          <span
            className={`${musicData.title.length > 10 ? "animate-scroll-text" : ""} inline-block whitespace-nowrap`}>
            {musicData.title}
          </span>
        </span>
      </div>
